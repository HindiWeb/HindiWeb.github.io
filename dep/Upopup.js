...
.hw-d-box { background-color: white; padding:0;max-width:99%;min-width:65vw;  border-radius:5px; border: 1px solid #27a; position:fixed; top:50%;left:50%; transform: translate(-50%,-50%); }
.hw-d-msg { padding : 30px 20px; overflow:auto;max-height:65vh;} 
...
.hw-d-btn-div {border-radius: 0 0 5px 5px; background:#444;padding:5px;border-top:1px solid #333;text-align: center;}
...
function output(buttonid,text,sec=2,c=null) { autoid += 1; createpopup(autoid,'BR'); showpopup(autoid,text,sec,'up 5',c); var div = document.getElementById(buttonid); if (!div) { div = buttonid; } var p = document.getElementById("popup"+autoid); p.style.position = 'absolute'; var rect = div.getBoundingClientRect(); var r = rect.right.toFixed(2); var b = rect.bottom.toFixed(2); var availWidth = window.innerWidth - r; p.style.top = ((b-2)+scrollY) +'px'; p.style.bottom = 'auto'; if (availWidth > 150 || text.length < 15) { p.style.left = r-2 + 'px'; p.style.right = 'auto'; } else { p.style.left = (r-200) + 'px'; p.style.right = 'auto'; } if (text.length > 50 ){ p.innerHTML += `<div class="popup-closer" onclick=hidePoAG('popup${autoid}')>[x]<div>`; } }

class Popup { constructor( parms = {}) { this.text = parms.text || null; this.pos = parms.pos || 'BR'; this.class = parms.class || null; this.move = parms.move || 'up 20'; this.sec = parms.sec || 2 ; this.id ; if (autoid ===211) { var noticeB = document.createElement('div');noticeB.id = 'noticeboard';document.body.appendChild(noticeB); noticeB.style.zIndex = 99; autoid +=1; }; if (parms.pos) {this.where = 'undefined';this.move = 'static'} else {this.where = 'noticeboard'}; this.create(); if (parms.sec) this.show();  }; static N = 0;  create(){ var pid = Popup.N++; this.id = `_${pid}`; return createpopup(`${this.id}`,this.pos,this.where); }  show(sec=null) { if (sec) {this.sec = sec }; showpopup(`${this.id}`,this.text,this.sec,this.move,this.class); } addClass(className){ var e = document.getElementById(`popup${this.id}`); e.classList.add(className); this.class = className; } setText(string) { this.text = string; } }

class HwAlert { constructor(options) { this.options = Object.assign({ title: 'Alert', message: '', buttons: ['OK'], callback: () => {}, overlay: 'on', }, options); this.createAlert(); this.createOverlay(); } createAlert() { const { title, message, buttons, callback } = this.options; this.alertCnt = document.createElement('div'); this.alertCnt.className = 'hw-alert'; this.alertCnt.innerHTML = ` <div class="hw-alert-content"> <h2>${title}</h2> <p>${message}</p> <div class="hw-alert-buttons"> ${buttons.map(btnText => `<button>${btnText}</button>`).join('')} </div> </div> `; this.buttons = this.alertCnt.querySelectorAll('.hw-alert-buttons button'); this.buttons.forEach((button, index) => button.addEventListener('click', () => this.btnClick(index))); document.body.appendChild(this.alertCnt); this.callback = callback; } createOverlay() { const { overlay } = this.options; if (overlay === 'on' || (Array.isArray(overlay) && overlay[0] === 'on')) { this.overlay = document.createElement('div'); this.overlay.className = 'hw-overlay'; if (!(Array.isArray(overlay) && overlay[1] === false)) { this.overlay.addEventListener('click', () => this.close()); } document.body.appendChild(this.overlay); } } updateMessage(newMessage) { this.options.message = newMessage; this.alertCnt.querySelector('.hw-alert-content p').textContent = newMessage; } updateBtns(newBtns) { this.options.buttons = newBtns; this.alertCnt.querySelector('.hw-alert-buttons').innerHTML = newBtns.map(btnText => `<button>${btnText}</button>`).join(''); this.buttons = this.alertCnt.querySelectorAll('.hw-alert-buttons button'); this.buttons.forEach((button, index) => button.addEventListener('click', () => this.btnClick(index))); } addBtn(newBtn) { this.options.buttons.push(newBtn); const buttonHtml = `<button>${newBtn}</button>`; this.alertCnt.querySelector('.hw-alert-buttons').innerHTML += buttonHtml; const newIndex = this.options.buttons.length - 1; const newBtnElement = this.alertCnt.querySelector(`.hw-alert-buttons button:nth-child(${newIndex + 1})`); newBtnElement.addEventListener('click', () => this.btnClick(newIndex)); } btnClick(index) { this.close(); this.callback(index); } open() { this.alertCnt.style.display = 'block'; if (this.overlay) { this.overlay.style.display = 'block'; } } close() { this.alertCnt.style.display = 'none'; if (this.overlay) { this.overlay.style.display = 'none'; } } }

class HwDialog { constructor(options) { this.options = Object.assign({ title: ' ', message: 'Welcome to HwDialog.', buttons: ['OK', 'Cancel'], callback: () => {}, dialogClass: '', msgClass: 'hw-d-msg', overlayClass: '', CSS: { title: {}, message: {}, dl: {} } }, options); this.createDialog(); } createDialog() { const { title, message, buttons, dialogClass, overlayClass, CSS } = this.options; const createElem = (tag, classes, textContent) => { const elem = document.createElement(tag); elem.className = classes; if(textContent) elem.innerHTML = textContent; return elem; }; const aStyl = (e,s) => { if (s) { Object.entries(s).forEach(([p, v]) => { e.style[p] = v; }); } }; const dC = newE('div', `hw-d-overlay ${overlayClass}`); const dlBox = newE('div', `hw-d-box ${dialogClass}`); const dlTitle = newE('h3', 'hwModalTitle', title); const dlMessage = newE('div', this.options.msgClass, message); const dlBtnCnt = newE('div', 'hw-d-btn-div'); buttons.forEach(btnText => { const button = newE('button', `hw-d-button ${this.options.buttonClass}`, btnText); button.addEventListener('click', () => this.btnClick(btnText)); dlBtnCnt.appendChild(button); }); try { aStyl(dlTitle, CSS.title); aStyl(dlMessage, CSS.message); aStyl(dlBox, CSS.dl); } catch (e) {;}  dlBox.append(dlTitle, dlMessage, dlBtnCnt); dC.appendChild(dlBox); document.body.appendChild(dC); this.dC = dC; } open() { if (this.dC) { this.dC.style.display = 'block'; } } hide() { if (this.dC) { this.dC.style.display = 'none'; } } close() {if (this.dC){ this.dC.remove();}} btnClick(btnText) { const index = this.options.buttons.indexOf(btnText); this.options.callback(index); } 
}

function hwPanel(title='hwPanel',msg) { var hwP = document.createElement('div'); hwP.className = 'hw-panel'; hwP.innerHTML = `<h3>${title}</h3><div>${msg}</div><span class="close-btn" onclick="this.parentNode.parentNode.removeChild(this.parentNode);"> &times;</span>`; document.body.appendChild(hwP); hwP.style.display = 'block'; return hwP; }